...
Option Explicit
' Copyright © 2003-2005 Karen Kenworthy
' All Rights Reserved
' http://www.karenware.com/
' Version 2.3 4/5/2005
...
Public Sub LvAdjust(lv As ListView)
    ...
    Dim adj As Boolean
    Dim lim As Long
    Dim ColHdrs As ColumnHeaders

    Set ColHdrs = lv.ColumnHeaders
    lim = ColHdrs.Count
    For i = 1 To lim
        adj = False
        If InStr(1, ColHdrs(i).Tag, "HIDDEN", vbTextCompare) > 0 Then
            ColHdrs(i).Width = 0
        End If
        If InStr(1, ColHdrs(i).Tag, LV_HIDE_TAG, vbTextCompare) > 0 Then
            ColHdrs(i).Width = 0
        End If
        If InStr(1, ColHdrs(i).Tag, LV_SORT_TAG, vbTextCompare) > 0 Then
            ColHdrs(i).Width = 0
        End If
        If ColHdrs(i).Width > 0 Then
            adj = True
