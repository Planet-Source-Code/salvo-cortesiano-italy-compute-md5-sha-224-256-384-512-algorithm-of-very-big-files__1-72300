...
Option Explicit
Option Compare Text
' Copyright © 2003-2007 Karen Kenworthy
' All Rights Reserved
' http://www.karenware.com/
' Version 2.91 6/8/2007

'Public fhCnt As Long
'Public fhCol As New Collection
...
Public ApiHelpFile As String
Public ApiForceAscii As Boolean ' set true if remote drive is known not to support Unicode
...
Public Const vbKeyF17 As Integer = vbKeyF16 + 1
Public Const vbKeyF18 As Integer = vbKeyF16 + 2
Public Const vbKeyF19 As Integer = vbKeyF16 + 3
Public Const vbKeyF20 As Integer = vbKeyF16 + 4
Public Const vbKeyF21 As Integer = vbKeyF16 + 5
Public Const vbKeyF22 As Integer = vbKeyF16 + 6
Public Const vbKeyF23 As Integer = vbKeyF16 + 7
Public Const vbKeyF24 As Integer = vbKeyF16 + 8
Public Const vbKeyLeftWin As Integer = &H5B
Public Const vbKeyRightWin As Integer = &H5C
Public Const vbKeyApps As Integer = &H5D
...
Public Enum VIRTUAL_KEY
    ...
    VK_LBUTTON = &H1& ' mouse buttons
    VK_MBUTTON = &H4&
    VK_RBUTTON = &H2&

    VK_LCONTROL = &HA2& ' control keys
    VK_CONTROL = &H11&
    VK_RCONTROL = &HA3&

    VK_LARROW = &H25&   ' arrow keys
    VK_RARROW = &H27&
    VK_DARROW = &H28&
    VK_UARROW = &H26&

    VK_LSHIFT = &HA0&   ' shift keys
    VK_SHIFT = &H10&
    VK_RSHIFT = &HA1&

    VK_LALT = &HA4  ' alt keys
    VK_ALT = &H12&
    VK_RALT = &HA5&
...
Public Const HOTKEYF_SHIFT As Integer = &H100
Public Const HOTKEYF_CONTROL As Integer = &H200
Public Const HOTKEYF_ALT As Integer = &H400
Public Const HOTKEYF_EXT As Integer = &H800
Public Const HOTKEYF_VK_MASK As Integer = &HFF&
...
Public Enum PT_SETTINGS_LOCATIONS
    PT_SETTINGS_DEFAULT
    PT_SETTINGS_USER_LOCAL = PT_SETTINGS_DEFAULT
    PT_SETTINGS_USER_ROAMING
    PT_SETTINGS_SHARED  ' All Users
    PT_SETTINGS_APPDIR
End Enum
...
Public Enum PT_ACCESS
    ...
    PT_ACCESS_READ = &H1
    PT_ACCESS_WRITE = &H2
    PT_ACCESS_MAX = PT_ACCESS_READ Or PT_ACCESS_WRITE
End Enum
...
Public Enum PT_API_ERRNUM
    PT_API_OK = 0
    PT_API_FILE_NOT_FOUND = vbObjectError + 1
    PT_API_INVALID
    PT_API_TOO_SMALL
    PT_API_UNKNOWN_VER
    PT_API_NO_TAG
    PT_API_WRONG_FILETYPE
    PT_API_MISSING_DLL
End Enum


Public Type PT_API_ERROR
    Num As PT_API_ERRNUM
    Desc As String
End Type
...
Public Enum PT_OS_WINVER
    PT_OS_VISTA = 600
End Enum

Public Type PT_OS_INFO
    Desc As String
    WinVer As PT_OS_WINVER   ' value of SDK header's WINVER (in decimal, not hex)
...
Public Type PT_LNK_FILE_INFO
    ...
    WorkDir As String
    Args As String
    ShowCmd As SHOW_COMMAND
    ShowCmdDesc As String
    HotKey As Integer
    HotKeyDesc As String
    IconFile As String
    IconIndex As Long
End Type
#End If
...
'============================================
Private Const API_FORCE_ASCII = False ' used to force ascii mode, when testing ascii api calls
...
Private Const ZB = (EB * 1024#) ' ZettaBytes
Private Const YB = (ZB * 1024#) ' YottaBytes
...
Private Const BCM_FIRST = &H1600                 ' Button control messages
...
Private Const MAX_UINT32 = 2 ^ 32
Private Const MAX_SINT32 = (2 ^ 31) - 1
Private Const MAX_UINT16 = 2 ^ 16
Private Const MAX_SINT16 = (2 ^ 15) - 1
...
' Commands to run HtmlHelp()
Private Enum HTML_HELP_COMMAND
    HH_ALINK_LOOKUP = &H13
    HH_CLOSE_ALL = &H12
    HH_DISPLAY_INDEX = &H2
    ...
    HH_DISPLAY_TOC = &H1
    HH_DISPLAY_TOPIC = &H0
    HH_ENUM_CATEGORY = &H15
    HH_ENUM_CATEGORY_IT = &H16
...
'    HH_MAX_TABS_CUSTOM = (HH_TAB_CUSTOM_LAST - HH_TAB_CUSTOM_FIRST + 1)
    HH_PRETRANSLATEMESSAGE = &HFD&
    HH_RESERVED1 = &HA
    HH_RESERVED2 = &HB
    HH_RESERVED3 = &HC
    HH_RESET_IT_FILTER = &H17
    HH_SET_EXCLUSIVE_FILTER = &H19
    HH_SET_GLOBAL_PROPERTY = &HFC&
    HH_SET_INCLUSIVE_FILTER = &H18
    HH_SET_INFO_TYPE = &H8
    HH_SET_WIN_TYPE = &H4
    HH_SYNC = &H9
...
'    FILE_GENERIC_READ = (STANDARD_RIGHTS_READ Or FILE_READ_DATA Or FILE_READ_ATTRIBUTES)
...
'    FILE_GENERIC_WRITE = (STANDARD_RIGHTS_WRITE Or FILE_WRITE_DATA Or FILE_WRITE_ATTRIBUTES Or FILE_APPEND_DATA)
...
Private Declare Function LookupAccountSidA Lib "advapi32.dll" ( _
...
Private Declare Function GetKeyState Lib "user32" ( _
    ByVal nVirtKey As VIRTUAL_KEY) As Integer
...
Private Declare Function GetTextCharacterExtra Lib "gdi32" ( _
    ByVal hdc As Long) As Long

Private Declare Function SetTextCharacterExtraA Lib "gdi32" ( _
    ByVal hdc As Long, _
...
Private Declare Function RemoveDirectoryW Lib "kernel32" ( _
    ByRef lpPathName As Byte) As Long
...
Private Declare Function WinHelpA Lib "user32.dll" ( _
    ...
    ByVal wCommand As HELP_COMMAND, _
    ByVal dwdata As Long) As Long

Private Declare Function HtmlHelpA Lib "hhctrl.ocx" ( _
     ByVal hwndCaller As Long, _
     ByVal pszFile As String, _
     ByVal ucommand As HTML_HELP_COMMAND, _
     ByVal dwdata As Long) As Long
...
Private GDFSW_OK As Boolean

Private WNGCW_OK As Boolean
'Private XParent_OK As Boolean

Private HtmlHelpFound As Boolean
...
Private Sub ApiInitShlwApi()
    ...
    If InitShlwApiDone Then Exit Sub
    If Not InitDone Then ApiInit
...
Private Sub ApiInitShell32()
    ...
    If InitShell32Done Then Exit Sub
    If Not InitDone Then ApiInit
...
Public Sub ApiInit()
    ...
    If GetProcAddress(hnd, "GetDiskFreeSpaceW") <> 0 Then GDFSW_OK = True
    ...
    If WinVer.Unicode Then ' GetVersionW worked, so check the rest
        ...
        GWDW_OK = True ' APIExists("kernel32", "GeetWindowsDirectoryW")
...
'    If WinVer.Major >= 5 Then XParent_OK = True
    ...
    If Len(App.HelpFile) > 0 Then
        ApiHelpFile = App.Path & "\" & App.HelpFile
        If ApiFileExists(ApiHelpFile) Then
            HelpEnabled = True
            If StrComp(Right(ApiHelpFile, 4), ".chm", vbTextCompare) = 0 Then
                HtmlHelpFound = True
            Else
                HtmlHelpFound = False
            End If
    ...
    If Not HelpEnabled Then
        hnd = LoadLibraryA("hhctrl.ocx")
        If hnd <> 0 Then
            ApiHelpFile = App.Path & "\" & App.Title & ".chm"
            If ApiFileExists(ApiHelpFile) Then
                HelpEnabled = True
                HtmlHelpFound = True
            Else
                ApiHelpFile = App.Path & "\PT" & App.Title & ".chm"
                If ApiFileExists(ApiHelpFile) Then
                    HelpEnabled = True
                    HtmlHelpFound = True
                End If
    ...
    If Not HelpEnabled Then
        ApiHelpFile = App.Path & "\" & App.Title & ".hlp"
        If ApiFileExists(ApiHelpFile) Then
            HelpEnabled = True
            HtmlHelpFound = False
        Else
            ApiHelpFile = App.Path & "\PT" & App.Title & ".hlp"
            If ApiFileExists(ApiHelpFile) Then
                HelpEnabled = True
                HtmlHelpFound = False
            End If
    ...
    End If
End Sub
Public Function ApiSettingsPath(Optional ByVal Location As PT_SETTINGS_LOCATIONS = PT_SETTINGS_DEFAULT) As String
    Dim s As String
    Dim ProdName As String
    Dim AppTitle As String

'    s = ApiSpecialFolder(CSIDL_APPDATA) ' user roaming
'    s = ApiSpecialFolder(CSIDL_LOCAL_APPDATA) ' user local
'    s = ApiSpecialFolder(CSIDL_COMMON_APPDATA) ' all users

'    CSIDL_APPDATA = &H1A&                ' <user name>\Application Data
'    CSIDL_LOCAL_APPDATA = &H1C&          ' <user name>\Local Settings\Applicaiton Data (non roaming)
'    CSIDL_COMMON_APPDATA = &H23&         ' All Users\Application Data

    If ApiWinVersion.WinVer < PT_OS_VISTA Then ' before Vista
        Location = PT_SETTINGS_APPDIR
    End If
    ProdName = App.ProductName
    AppTitle = Replace(App.Title, "Karen's ", "", , , vbTextCompare)

    Select Case Location
        Case PT_SETTINGS_APPDIR
            s = App.Path
            If Right(s, 1) <> "\" Then s = s & "\"
            ApiSettingsPath = s
            Exit Function

        Case PT_SETTINGS_USER_LOCAL
            s = ApiSpecialFolder(CSIDL_LOCAL_APPDATA)
            If Len(s) > 0 Then
                If Right(s, 1) <> "\" Then s = s & "\"
                s = s & ProdName & "\" & AppTitle & "\"
                If ApiFolderExists(s) Then
                    ApiSettingsPath = s
                    Exit Function
                End If
                ApiFolderCreateEx s, PT_ATTR_NORMAL
                If ApiFolderExists(s) Then
                    ApiSettingsPath = s
                    Exit Function
                End If
        ...
        Case PT_SETTINGS_USER_ROAMING
            s = ApiSpecialFolder(CSIDL_APPDATA)
            If Len(s) > 0 Then
                If Right(s, 1) <> "\" Then s = s & "\"
                s = s & ProdName & "\" & AppTitle & "\"
                If ApiFolderExists(s) Then
        ...
        Case PT_SETTINGS_SHARED
            s = ApiSpecialFolder(CSIDL_COMMON_APPDATA)
            If Len(s) > 0 Then
                If Right(s, 1) <> "\" Then s = s & "\"
                s = s & ProdName & "\" & AppTitle & "\"
                If ApiFolderExists(s) Then
    ...
    ApiSettingsPath = ""
End Function
Public Function ApiSettingsFileName(Filename As String) As String
    ApiSettingsFileName = ApiSettingsPath(PT_SETTINGS_DEFAULT) & Filename
End Function
Public Function ApiSettingsOpenInput(Filename As String) As Long
    Dim fn As Long

    fn = FreeFile()
    On Error Resume Next
    Open ApiSettingsFileName(Filename) For Input Access Read As fn
    ApiSettingsOpenInput = fn
End Function
Public Function ApiSettingsOpenOutput(Filename As String) As Long
    Dim fn As Long

    fn = FreeFile()
    On Error Resume Next
    Open ApiSettingsFileName(Filename) For Output Access Write As fn
    ApiSettingsOpenOutput = fn
End Function
Public Sub ApiSettingsMigrate(OldFileName As String, Optional NewFileName As String = "")
    If ApiWinVersion.WinVer < PT_OS_VISTA Then  ' before Vista
        Exit Sub
    End If

    If Len(OldFileName) <= 0 Then Exit Sub
    If Len(NewFileName) <= 0 Then NewFileName = OldFileName

    Dim OldFullName As String
    Dim NewFullName As String

    OldFullName = ApiSettingsPath(PT_SETTINGS_APPDIR) & OldFileName
    NewFullName = ApiSettingsPath(PT_SETTINGS_DEFAULT) & NewFileName
    If StrComp(OldFullName, NewFullName, vbTextCompare) = 0 Then Exit Sub

    If ApiFileExists(NewFullName) Then Exit Sub
    If Not ApiFileExists(OldFullName) Then Exit Sub

    On Error Resume Next
    FileCopy OldFullName, NewFullName
    ...
    Err.Clear
End Sub
Public Sub ApiSetError(ByRef Error As PT_API_ERROR)
    Select Case Error.Num
        Case PT_API_OK:             Error.Desc = ""
        Case PT_API_FILE_NOT_FOUND: Error.Desc = "File Not Found"
        Case PT_API_INVALID:        Error.Desc = "Invalid File Format"
        Case PT_API_NO_TAG:         Error.Desc = "Missing Tag"
        Case PT_API_TOO_SMALL:      Error.Desc = "File Too Small"
        Case PT_API_UNKNOWN_VER:    Error.Desc = "Unknown File Version"
        Case PT_API_WRONG_FILETYPE: Error.Desc = "Wrong File Type"
        Case PT_API_MISSING_DLL:    Error.Desc = "Required DLL is Missing"
        Case Else:                  Error.Desc = "Unknown Error (" & CStr(Error.Num) & ")"
    End Select
End Sub
...
Public Function ApiHelpTopic(TopicID As Long) As Boolean
    ...
    If HtmlHelpFound Then
        HtmlHelpA GetDesktopWindow(), ApiHelpFile, HH_DISPLAY_INDEX, 0
        HtmlHelpA GetDesktopWindow(), ApiHelpFile, HH_HELP_CONTEXT, TopicID
    Else
        WinHelpA GetDesktopWindow(), ApiHelpFile, HELP_CONTEXT, TopicID
    End If
    ApiHelpTopic = True
End Function
Public Function ApiHelpIndex() As Boolean
    ...
    If HtmlHelpFound Then
        HtmlHelpA GetDesktopWindow(), ApiHelpFile, HH_DISPLAY_INDEX, 0
    Else
        WinHelpA GetDesktopWindow(), ApiHelpFile, HELP_FINDER, 0
    End If
    ApiHelpIndex = True
End Function
Public Function ApiHelpContents() As Boolean
    ...
    If HtmlHelpFound Then
        HtmlHelpA GetDesktopWindow(), ApiHelpFile, HH_DISPLAY_TOC, 0
    Else
        WinHelpA GetDesktopWindow(), ApiHelpFile, HELP_FINDER, 0
    End If
    ApiHelpContents = True
End Function
Public Function ApiHelpFinder() As Boolean
    If Not InitDone Then ApiInit
    If Not HelpEnabled Then Exit Function

    If HtmlHelpFound Then
        HtmlHelpA GetDesktopWindow(), ApiHelpFile, HH_HELP_FINDER, 0
    Else
        WinHelpA GetDesktopWindow(), ApiHelpFile, HELP_FINDER, 0
    End If
    ApiHelpFinder = True
...
Public Sub APiHelpQuit()
    If Not InitDone Then ApiInit
    If Not HelpEnabled Then Exit Sub
    
    If HtmlHelpFound Then
        HtmlHelpA 0, vbNullString, HH_CLOSE_ALL, 0
    Else
        WinHelpA 0, vbNullString, HELP_QUIT, 0
    End If
End Sub
Public Function ApiKeyDown(VirtKey As VIRTUAL_KEY) As Boolean
    Dim KeyDown As Integer

    KeyDown = GetKeyState(VirtKey)
    If KeyDown < 0 Then ' high-order bit set
        ApiKeyDown = True
    Else
        ApiKeyDown = False
    End If
End Function
Public Function ApiKeyToggle(VirtKey As VIRTUAL_KEY) As Boolean
    Dim KeyToggle As Integer

    KeyToggle = GetKeyState(VirtKey)
    If (KeyToggle And &H1) = 1 Then
        ApiKeyToggle = True
    Else
        ApiKeyToggle = False
    End If
...
'        ptoi.Unicode = False
    ...
    Desc = Desc & " v" & CStr(ptoi.Major) & "." & CStr(ptoi.Minor) & "." & CStr(ptoi.Build)
    ptoi.Desc = Desc
...
'   set WIN32_WINNT
    If ptoi.WinVista Then
        ptoi.WIN32_WINNT = 601
    ElseIf ptoi.Win2K3 Then
...
End Function
Public Function ApiShortcutInfo(LnkFile As String) As PT_LNK_FILE_INFO
    Dim pidl As Long
    Dim fd As SL_FIND_DATAA
    Dim cShellLink As PTShellLink
    Dim cPersistFile As PTPersistFile
    Dim lfi As PT_LNK_FILE_INFO
    Dim s As String
    Dim hk As Long
    ...
    s = Space(MAX_PATH)
    cShellLink.GetPath s, Len(s), fd, SLGP_UNCPRIORITY
    lfi.ExeFile = ApiTextStrip(s)

    s = Space(MAX_PATH)
    cShellLink.GetWorkingDirectory s, Len(s)
    lfi.WorkDir = ApiTextStrip(s)

    s = Space(MAX_PATH)
    cShellLink.GetArguments s, Len(s)
    lfi.Args = ApiTextStrip(s)

    s = Space(MAX_PATH)
    cShellLink.GetDescription s, Len(s)
    lfi.Desc = ApiTextStrip(s)

    cShellLink.GetHotkey hk
    lfi.HotKey = hk
    lfi.HotKeyDesc = ApiDispHotKey(lfi.HotKey)

    s = Space(MAX_PATH)
    lfi.IconIndex = 0
    cShellLink.GetIconLocation s, Len(s), lfi.IconIndex
    lfi.IconFile = ApiTextStrip(s)
    If Len(lfi.IconFile) <= 0 Then
        lfi.IconFile = lfi.ExeFile
        lfi.IconIndex = 0
    End If

    cShellLink.GetIDList pidl
    cShellLink.GetShowCmd lfi.ShowCmd
    lfi.ShowCmdDesc = ApiDispShowCmd(lfi.ShowCmd)
    lfi.ErrNum = 0
...
ErrHandler:
    lfi.ErrNum = Err.Number
    If lfi.ErrNum = 0 Then
        lfi.ErrNum = -1
    Else
        lfi.ErrDesc = Err.Description
    End If
    ...
    ApiShortcutInfo = lfi
End Function
Public Function ApiDispHotKey(HotKey As Integer) As String
    Dim s As String

    s = ""
    If (HotKey And HOTKEYF_SHIFT) > 0 Then
        If Len(s) > 0 Then s = s & "+"
        s = s & "Shift"
    End If
    If (HotKey And HOTKEYF_CONTROL) > 0 Then
        If Len(s) > 0 Then s = s & "+"
        s = s & "Ctrl"
    End If
    If (HotKey And HOTKEYF_ALT) > 0 Then
        If Len(s) > 0 Then s = s & "+"
        s = s & "Alt"
    End If
    If (HotKey And HOTKEYF_EXT) > 0 Then
        ' extended key, i.e. down arrow
    End If

    If Len(s) > 0 Then s = s & "+"

    Select Case HotKey And HOTKEYF_VK_MASK
        Case vbKeyAdd:          s = s & "Num +"
        Case vbKeyBack:         s = s & "Backspace"
        Case vbKeyCancel:       s = s & "Cancel"
        Case vbKeyCapital:      s = s & "Caps Lock"
        Case vbKeyClear:        s = s & "Clear"
        Case vbKeyControl:      s = s & "Ctrl"
        Case vbKeyDecimal:      s = s & "Num Del"
        Case vbKeyDelete:       s = s & "Delete"
        Case vbKeyDivide:       s = s & "Num /"
        Case vbKeyDown:         s = s & "Down"
        Case vbKeyEnd:          s = s & "End"
        Case vbKeyEscape:       s = s & "Esc"
        Case vbKeyExecute:      s = s & "Execute"
        Case vbKeyF1 To vbKeyF24:
            s = s & "F" & CStr((HotKey And HOTKEYF_VK_MASK) - vbKeyF1 + 1)
        Case vbKeyHelp:         s = s & "HELP"
        Case vbKeyHome:         s = s & "Home"
        Case vbKeyInsert:       s = s & "Insert"
        Case vbKeyLeft:         s = s & "Left"
        Case vbKeyMenu:         s = s & "Menu"
        Case vbKeyMultiply:     s = s & "Num *"
        Case vbKeyNumlock:      s = s & "Num Lock"
        Case vbKeyNumpad0 To vbKeyNumpad9:
            s = s & "Num " & CStr((HotKey And HOTKEYF_VK_MASK) - vbKeyNumpad0 + 1)
        Case vbKeyPageDown:     s = s & "Page Down"
        Case vbKeyPageUp:       s = s & "Page Up"
        Case vbKeyPause:        s = s & "Pause"
        Case vbKeyPrint:        s = s & "Print"
        Case vbKeyReturn:       s = s & "Return"
        Case vbKeyRight:        s = s & "Right"
        Case vbKeyScrollLock:   s = s & "Scroll Lock"
        Case vbKeySelect:       s = s & "Select"
        Case vbKeySeparator:    s = s & "Separator"
        Case vbKeyShift:        s = s & "Shift"
        Case vbKeySnapshot:     s = s & "Snapshot"
        Case vbKeySubtract:     s = s & "Num -"
        Case vbKeyTab:          s = s & "Tab"
        Case vbKeyUp:           s = s & "Up"
        Case Else:              s = s & Chr(HotKey And HOTKEYF_VK_MASK)
    End Select

    ApiDispHotKey = s
End Function
#End If ' API_SHORTCUT
Public Function ApiDispShowCmd(ShowCmd As SHOW_COMMAND) As String
    Dim s As String

    s = ""
    Select Case ShowCmd
        Case SW_HIDE:               s = "Hidden"
        Case SW_NORMAL:             s = "Normal"    ' 1
        Case SW_SHOWMINIMIZED:      s = "Minimized"  ' 2
        Case SW_MAXIMIZE:           s = "Maximized"   ' 3
        Case SW_SHOWMAXIMIZED:      s = "Maximized"  ' 3
        Case SW_SHOWNOACTIVATE:     s = "Normal - Not Activated" ' 4
        Case SW_SHOW:               s = "Normal"  ' 5
        Case SW_MINIMIZE:           s = "Minimized"   ' 6
        Case SW_SHOWMINNOACTIVE:    s = "Minimized - Not Activated" ' 7
        Case SW_SHOWNA:             s = "Normal - Not Activated"    ' 8
        Case SW_RESTORE:            s = "Restored" ' 9
        Case SW_SHOWDEFAULT:        s = "Default"  ' 10
        Case Else:                  s = "Unknown (" & CStr(ShowCmd) & ")"
    End Select
...
Public Function ApiAttrsGet(Fid As String) As PT_ATTR
    ...
    If GFAW_OK And (Not ApiForceAscii) Then
        uba = UnicodeFid(Fid)
...
Public Function ApiAttrsSet(Fid As String, Attrs As PT_ATTR) As Long
    ...
    If SFAW_OK And (Not ApiForceAscii) Then
        uba = UnicodeFid(Fid)
...
End Function
Public Function ApiDispFlagsAndAttrs(Code As Long) As String
    Dim s As String
...
'    FILE_FLAG_RANDOM_ACCESS = &H10000000
    ...
    s = ""
    If (Code And FILE_FLAG_SEQUENTIAL_SCAN) = FILE_FLAG_SEQUENTIAL_SCAN Then
        If Len(s) > 0 Then s = s & " Or "
        s = s & "FILE_FLAG_SEQUENTIAL_SCAN"
    End If
    If (Code And FILE_FLAG_RANDOM_ACCESS) = FILE_FLAG_RANDOM_ACCESS Then
        If Len(s) > 0 Then s = s & " Or "
        s = s & "FILE_FLAG_RANDOM_ACCESS"
    End If
...
'    FILE_ATTRIBUTE_ENCRYPTED = &H4000
    ...
    If (Code And FILE_ATTRIBUTE_READONLY) = FILE_ATTRIBUTE_READONLY Then
        If Len(s) > 0 Then s = s & " Or "
        s = s & "FILE_ATTRIBUTE_READONLY"
    End If
    If (Code And FILE_ATTRIBUTE_HIDDEN) = FILE_ATTRIBUTE_HIDDEN Then
        If Len(s) > 0 Then s = s & " Or "
        s = s & "FILE_ATTRIBUTE_HIDDEN"
    End If
    If (Code And FILE_ATTRIBUTE_SYSTEM) = FILE_ATTRIBUTE_SYSTEM Then
        If Len(s) > 0 Then s = s & " Or "
        s = s & "FILE_ATTRIBUTE_SYSTEM"
    End If
    If (Code And FILE_ATTRIBUTE_DIRECTORY) = FILE_ATTRIBUTE_DIRECTORY Then
        If Len(s) > 0 Then s = s & " Or "
        s = s & "FILE_ATTRIBUTE_DIRECTORY"
    End If
    If (Code And FILE_ATTRIBUTE_ARCHIVE) = FILE_ATTRIBUTE_ARCHIVE Then
        If Len(s) > 0 Then s = s & " Or "
        s = s & "FILE_ATTRIBUTE_ARCHIVE"
    End If
    If (Code And FILE_ATTRIBUTE_DEVICE) = FILE_ATTRIBUTE_DEVICE Then
        If Len(s) > 0 Then s = s & " Or "
        s = s & "FILE_ATTRIBUTE_DEVICE"
    End If
    If (Code And FILE_ATTRIBUTE_NORMAL) = FILE_ATTRIBUTE_NORMAL Then
        If Len(s) > 0 Then s = s & " Or "
        s = s & "FILE_ATTRIBUTE_NORMAL"
    End If
    If (Code And FILE_ATTRIBUTE_TEMPORARY) = FILE_ATTRIBUTE_TEMPORARY Then
        If Len(s) > 0 Then s = s & " Or "
        s = s & "FILE_ATTRIBUTE_TEMPORARY"
    End If
    If (Code And FILE_ATTRIBUTE_SPARSE_FILE) = FILE_ATTRIBUTE_SPARSE_FILE Then
        If Len(s) > 0 Then s = s & " Or "
        s = s & "FILE_ATTRIBUTE_SPARSE_FILE"
    End If
    If (Code And FILE_ATTRIBUTE_REPARSE_POINT) = FILE_ATTRIBUTE_REPARSE_POINT Then
        If Len(s) > 0 Then s = s & " Or "
        s = s & "FILE_ATTRIBUTE_REPARSE_POINT"
    End If
    If (Code And FILE_ATTRIBUTE_COMPRESSED) = FILE_ATTRIBUTE_COMPRESSED Then
        If Len(s) > 0 Then s = s & " Or "
        s = s & "FILE_ATTRIBUTE_COMPRESSED"
    End If
    If (Code And FILE_ATTRIBUTE_OFFLINE) = FILE_ATTRIBUTE_OFFLINE Then
        If Len(s) > 0 Then s = s & " Or "
        s = s & "FILE_ATTRIBUTE_OFFLINE"
    End If
    If (Code And FILE_ATTRIBUTE_NOT_CONTENT_INDEXED) = FILE_ATTRIBUTE_NOT_CONTENT_INDEXED Then
        If Len(s) > 0 Then s = s & " Or "
        s = s & "FILE_ATTRIBUTE_NOT_CONTENT_INDEXED"
    End If
    If (Code And FILE_ATTRIBUTE_ENCRYPTED) = FILE_ATTRIBUTE_ENCRYPTED Then
        If Len(s) > 0 Then s = s & " Or "
        s = s & "FILE_ATTRIBUTE_ENCRYPTED"
    End If

    s = "&H" & Hex(Code) & ": " & s
    ApiDispFlagsAndAttrs = s
End Function
Public Function ApiFileRecycle(Filename As String) As Boolean
    ...
    If SHFOW_OK And (Not ApiForceAscii) Then  ' unicode
        SHFOSW.wFunc = FO_DELETE
        uba = Filename & vbNullChar & vbNullChar ' UnicodeFid(Filename & vbNullChar & vbNullChar)
...
Public Function ApiFolderRecycle(FolderName As String) As Boolean
    ...
    If SHFOW_OK And (Not ApiForceAscii) Then
        SHFOSW.wFunc = FO_DELETE
...
Private Sub ParseFileNames(ByRef fi As PT_FILE_INFO)
    ...
    If Len(fi.AltName) <= 0 Then
        If GSPNW_OK And (Not ApiForceAscii) Then
            uba = UnicodeFid(fi.FullName)
            buflen = (UBound(uba) + 1) / 2
...
Private Sub ParseFileTime(ByRef ft As FILETIME, ByRef pft As PT_FILE_TIME)
    ...
    pft.TimeUTC = DateSerial(st.wYear, st.wMonth, st.wDay) + TimeSerial(st.wHour, st.wMinute, st.wSecond)
    If Err.Number <> 0 Then
        pft.Invalid = True
        Err.Clear
    End If
    ...
    pft.TimeLocal = DateSerial(st.wYear, st.wMonth, st.wDay) + TimeSerial(st.wHour, st.wMinute, st.wSecond)
    If Err.Number <> 0 Then
        pft.Invalid = True
        Err.Clear
    End If
...
Private Sub ParseFileSizes(nFileSizeLow As Long, nFileSizeHigh As Long, ByRef fi As PT_FILE_INFO)
    ...
    End If
End Sub
Public Function ApiFindFiles(Folder As String, Mask As String) As PT_FID_ARRAY
    ...
    Dim i As Long
    Dim lim As Long
    Dim SwapFid As String
    Dim Sorted As Boolean

    FileCnt = 0
    result = ApiFindFirstFile(fd, Folder, Mask)
    Do While fd.Continue
        If (fd.Attrs And PT_ATTR_DIRECTORY) = 0 Then
            ReDim Preserve fa.Fids(FileCnt)
            fa.Fids(FileCnt) = fd.FullName
            FileCnt = FileCnt + 1
        End If
        ApiFindNextFile fd
    Loop
    ...
    If FileCnt > 1 Then ' check for sort order
        lim = FileCnt - 1
        Sorted = False
        Do While Not Sorted
            Sorted = True
            For i = 1 To lim
                If StrComp(fa.Fids(i), fa.Fids(i - 1), vbTextCompare) < 0 Then
                    SwapFid = fa.Fids(i)
                    fa.Fids(i) = fa.Fids(i - 1)
                    fa.Fids(i - 1) = SwapFid
                    Sorted = False
                End If
            Next i
    ...
    ApiFindFiles = fa
End Function
Public Function ApiFindFolders(Folder As String, Mask As String) As PT_FID_ARRAY
    Dim fa As PT_FID_ARRAY
    ...
    result = ApiFindFirstFile(fd, Folder, Mask)
    Do While fd.Continue
        If (fd.Attrs And PT_ATTR_DIRECTORY) = PT_ATTR_DIRECTORY Then
            If (fd.Name <> ".") And (fd.Name <> "..") Then
                ReDim Preserve fa.Fids(FolderCnt)
                s = fd.FullName
                If Right(s, 1) <> "\" Then s = s & "\"
                fa.Fids(FolderCnt) = s
    ...
    If FolderCnt > 1 Then ' check for sort order
        lim = FolderCnt - 1
        Sorted = False
        Do While Not Sorted
            Sorted = True
    ...
    fa.FidCnt = FolderCnt
    fa.FolderCnt = FolderCnt
    fa.FileCnt = 0
    ApiFindFolders = fa
End Function
Public Function ApiFindAll(Folder As String, Mask As String) As PT_FID_ARRAY
    Dim fa As PT_FID_ARRAY
    Dim FidCnt As Long
    Dim FileCnt As Long
    Dim FolderCnt As Long
    ...
    If FidCnt > 1 Then ' check for sort order
        lim = FidCnt - 1
        Sorted = False
        Do While Not Sorted
            Sorted = True
    ...
    ApiFindAll = fa
End Function
Public Function ApiFindFirstFile(ByRef fd As PT_FILE_INFO, Optional Folder As String = "", Optional Masks As String = "") As Long
    Dim uba() As Byte
    Dim Unicode As String
    ...
    If fd.MaskInx <= 0 Then ' first time
        If Len(Folder) <= 0 Then
            fd.Continue = False
            ApiFindFirstFile = 0
            Exit Function
        ElseIf Len(Masks) > 0 Then ' normal find
            If Right(Folder, 1) <> "\" Then Folder = Folder & "\"
            fd.Masks = Split(Masks, ";")
            fd.MaskLim = UBound(fd.Masks)
            fd.MaskInx = 0
        Else    ' file exists check
            ReDim fd.Masks(0)
            fd.Masks(0) = ""
            fd.MaskLim = 0
            fd.MaskInx = 0
    ...
    fd.FindMask = Folder & fd.Masks(fd.MaskInx)
    ParseFID fd.FindMask, fd ' sets .Protocol and .Folder
    ...
    Do
        Retry = False
        If FFFW_OK And (Not ApiForceAscii) Then
            uba = UnicodeFid(fd.FindMask)
            result = FindFirstFileW(uba(0), fdw)
            If result <> INVALID_HANDLE_VALUE Then
'fhCnt = fhCnt + 1
'fhCol.Add fd.FindMask, "K" & CStr(Result)
            ...
            End If
        Else
            result = FindFirstFileA(fd.FindMask, fda)
            If result <> INVALID_HANDLE_VALUE Then
'fhCnt = fhCnt + 1
'fhCol.Add fd.FindMask, "K" & CStr(Result)
        ...
        If result = INVALID_HANDLE_VALUE Then
            fd.MaskInx = fd.MaskInx + 1
            If fd.MaskInx > fd.MaskLim Then
                SetApiError
                ApiFindClose fd ' just in case
'                fd.Continue = False
'                fd.Handle = 0
...
Public Function ApiFindNextFile(ByRef fd As PT_FILE_INFO) As Long
    ...
    If FNFW_OK And (Not ApiForceAscii) Then
...
'        fd.Handle = 0
'        fd.Continue = False
        If (ApiLastError = ERROR_NO_MORE_FILES) Or (ApiLastError = 0) Then
            If fd.MaskInx < fd.MaskLim Then
                fd.MaskInx = fd.MaskInx + 1
                result = ApiFindFirstFile(fd)
            End If
            fd.Error = False
            fd.ErrorNum = 0
        Else
...
Public Function ApiFileExists(Filename As String) As Boolean
    ...
    fh = ApiFindFirstFile(fd, Filename)
...
Private Function LongName(Fid As String) As String
    ...
    If GLPNW_OK And (Not ApiForceAscii) Then
        suba = UnicodeFid(Fid)
...
Public Function ApiFileSpace(ByRef fi As PT_FILE_INFO) As Variant
    ...
    If GCFSW_OK And (Not ApiForceAscii) Then
        uba = UnicodeFid(fi.FullName)
...
Public Function ApiFileInfo(Fid As String) As PT_FILE_INFO
    ...
    If GFAXW_OK And (Not ApiForceAscii) Then
        uba = UnicodeFid(Fid)
...
Private Function ApiFileActualSize(Fid As String) As Variant
    ...
    sa.nLength = 0
    If CFW_OK And (Not ApiForceAscii) Then
        uba = UnicodeFid(Fid)
        fh = CreateFileW(uba(0), GENERIC_READ, FILE_SHARE_READ Or FILE_SHARE_WRITE, 0, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS Or FILE_FLAG_SEQUENTIAL_SCAN, 0)
    Else
        fh = CreateFileA(Fid, GENERIC_READ, FILE_SHARE_READ Or FILE_SHARE_WRITE, 0, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS Or FILE_FLAG_SEQUENTIAL_SCAN, 0)
    End If
    If fh = INVALID_HANDLE_VALUE Then
        SetApiError
...
Public Function ApiFolderCurrent() As String
    ...
    If GCDW_OK And (Not ApiForceAscii) Then
...
Public Function ApiFolderOpen(Fid As String, Optional Attrs As PT_ATTR = PT_ATTR_NORMAL) As Long
    ...
    If CFW_OK And (Not ApiForceAscii) Then
        uba = UnicodeFid(Fid)
        fh = CreateFileW(uba(0), DesiredAccess, ShareMode, 0, CreateDisposition, FlagsAndAttrs, 0)
    Else
        fh = CreateFileA(Fid, DesiredAccess, ShareMode, 0, CreateDisposition, FlagsAndAttrs, 0)
    End If
...
End Function
Public Function ApiFolderCreateEx(ByVal FullName As String, Optional Attrs As PT_ATTR = PT_ATTR_NORMAL) As Boolean
    Dim uba() As Byte
    Dim sa As SECURITY_ATTRIBUTES
    Dim result As Long
    ...
    sa.nLength = 0
    If CFW_OK And (Not ApiForceAscii) Then
        uba = UnicodeFid(FullName)
        result = CreateDirectoryW(uba(0), sa)
    Else
        result = CreateDirectoryA(FullName, sa)
    End If

    If result = 0 Then ' error
        SetApiError
        If ApiLastError = 183 Then ' already exists
            ApiFolderCreateEx = False
'            Exit Function
        Else ' try recursion
            Dim i As Long
            i = InStrRev(FullName, "\", Len(FullName) - 1)
            If i > Len(ApiFolderRoot(FullName)) Then
                If ApiFolderCreateEx(Left(FullName, i), Attrs) Then
                    ApiFolderCreateEx FullName, Attrs
                Else
                    ApiFolderCreateEx = False
                    Exit Function
                End If
    ...
    ApiAttrsSet FullName, Attrs
    ApiLastError = 0
    ApiFolderCreateEx = True
End Function
...
Public Function ApiFolderSelect(Title As String, CurrentDir As String) As String
    ...
    If SHBFFW_OK And (Not ApiForceAscii) Then
...
Public Function ApiFileOpenOutput(Fid As String)
    ApiFileOpenOutput = ApiFileOpen(Fid, PT_ACCESS_WRITE, PT_ACCESS_READ, PT_ATTR_NORMAL, True, True, False)
End Function
Private Function ApiFileOpen(Fid As String, Optional MyAccess As PT_ACCESS = PT_ACCESS_READ, Optional OtherAccess As PT_ACCESS = PT_ACCESS_READ, Optional Attrs As PT_ATTR = PT_ATTR_NORMAL, Optional Overwrite As Boolean = False, Optional Sequential As Boolean = True, Optional Backup As Boolean = False) As Long
    ...
    Dim DesiredAccess9x As DESIRED_ACCESS
    Dim FlagsAndAttrs9x As FILE_FLAG_AND_ATTR

    If Not InitDone Then ApiInit

    DesiredAccess = 0
    DesiredAccess9x = 0
    If (MyAccess And PT_ACCESS_READ) = PT_ACCESS_READ Then
        DesiredAccess = DesiredAccess Or GENERIC_READ ' FILE_GENERIC_READ
        If WinVer.Win9x Then
            DesiredAccess9x = DesiredAccess9x Or GENERIC_READ
        End If
    End If

    If (MyAccess And PT_ACCESS_WRITE) = PT_ACCESS_WRITE Then
        DesiredAccess = DesiredAccess Or GENERIC_WRITE ' FILE_GENERIC_WRITE
        If WinVer.Win9x Then
            DesiredAccess9x = DesiredAccess9x Or GENERIC_WRITE
        End If
    End If

    ShareMode = 0
    If (OtherAccess And PT_ACCESS_READ) = PT_ACCESS_READ Then
        ShareMode = ShareMode Or FILE_SHARE_READ
    End If
    If (OtherAccess And PT_ACCESS_WRITE) = PT_ACCESS_WRITE Then
    ...
    If Backup And (Not WinVer.Win9x) Then
...
'fhSrc = CreateFile(Src, GENERIC_READ, _
'    FILE_SHARE_READ, _
...
'    0, 0, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0)
    ...
    If CFW_OK And (Not ApiForceAscii) Then
        ...
        End If
    Else
        fh = CreateFileA(Fid, DesiredAccess9x, ShareMode, 0, CreateDisposition, FlagsAndAttrs, 0)
        If fh = INVALID_HANDLE_VALUE Then
            ApiSleep 3000  ' wait 3 seconds and try again
            fh = CreateFileA(Fid, DesiredAccess9x, ShareMode, 0, CreateDisposition, FlagsAndAttrs, 0)
        End If
...
Private Function ApiStreamInfo(Fid As String) As PT_STREAM_INFO()
    ...
    sa.nLength = 0
    fh = CreateFileA(Fid, GENERIC_READ, 0, 0, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS Or FILE_FLAG_SEQUENTIAL_SCAN, 0)
    If fh = INVALID_HANDLE_VALUE Then
        SetApiError
        ApiStreamInfo = si
        Exit Function
...
Private Function FileCopyEx(ByRef sfi As PT_FILE_INFO, ByRef dfi As PT_FILE_INFO) As PT_COPY_RESULT
    ...
    If CFXW_OK And (Not ApiForceAscii) Then
...
Private Function FileBackup(ByRef sfi As PT_FILE_INFO, ByRef dfi As PT_FILE_INFO) As PT_COPY_RESULT
    ...
    sa.nLength = 0
    fhSrc = CreateFileA(sfi.FullName, GENERIC_READ, 0, 0, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS Or FILE_FLAG_SEQUENTIAL_SCAN, 0)
    If fhSrc = INVALID_HANDLE_VALUE Then
    ...
    fhDst = CreateFileA(dfi.FullName, GENERIC_WRITE Or WRITE_OWNER Or WRITE_DAC, FILE_SHARE_READ Or FILE_SHARE_WRITE, 0, CREATE_ALWAYS, FILE_FLAG_BACKUP_SEMANTICS Or FILE_FLAG_SEQUENTIAL_SCAN, 0)
    If fhDst = INVALID_HANDLE_VALUE Then
...
Public Function ApiFileDelete(Filename As String) As Long
    ...
    If DFW_OK And (Not ApiForceAscii) Then
...
Public Function ApiFolderDelete(FolderName As String) As Long
    ...
    If DFW_OK And (Not ApiForceAscii) Then
        uba = UnicodeFid(s)
        ok = RemoveDirectoryW(uba(0)) ' DeleteFileW(uba(0))
    Else
        ok = RemoveDirectoryA(s) ' DeleteFileA(s) & vbNullChar
    End If
...
End Sub
Public Sub ApiFormCaption(Frm As Form)
    Dim Ctrl As Control
    Dim i As Long
    Dim s As String
    Dim sa() As String
    ...
    For Each Ctrl In Frm.Controls
        ...
        If Len(s) > 0 Then
            ...
            lim = UBound(sa)
            For i = 0 To lim
                If IsNumeric(sa(i)) Then
                    s = ""
                    s = LoadResString(CLng(sa(i) & "0"))
                    If Len(s) > 0 Then Ctrl.Caption = Replace(s, "\n", vbCrLf, , , vbTextCompare)
                    s = ""
                    s = LoadResString(CLng(sa(i) & "1"))
                    If Len(s) > 0 Then Ctrl.ToolTipText = Replace(s, "\n", vbCrLf, , , vbTextCompare)
                End If
            Next i
        End If
        Err.Clear
...
Public Sub ApiFormFont(Frm As Form, Optional FontName As String = "Arial", Optional MinFontSize As Single = 9)
    ...
    For Each Ctrl In Frm.Controls
        ...
        If InStr(1, Ctrl.Tag, "NOSIZE", vbTextCompare) <= 0 Then
            ...
            End If
        End If
        s = Replace(Ctrl.Caption, "\n", vbCrLf, , , vbTextCompare)
        Ctrl.Caption = Replace(s, "(c)", "©")
        If StrComp(TypeName(Ctrl), "StatusBar", vbTextCompare) = 0 Then
            ...
            For i = 1 To Ctrl.Panels.Count
                ...
                End If
            Next i
        ElseIf StrComp(TypeName(Ctrl), "ListBox", vbTextCompare) = 0 Then
            Ctrl.Height = OrigHeight
        ElseIf StrComp(TypeName(Ctrl), "DirListBox", vbTextCompare) = 0 Then
            Ctrl.Height = OrigHeight
        End If
        Err.Clear
    Next Ctrl
    ...
    On Error GoTo 0
End Sub
Public Sub ApiFormTop(hwnd As Long)
...
End Sub
Public Sub ApiFormTopMost(hwnd As Long)
    SetWindowPos hwnd, HWND_TOPMOST, 0, 0, 0, 0, SWP_FLAGS
End Sub
Public Sub ApiFormNoTopMost(hwnd As Long)
    SetWindowPos hwnd, HWND_NOTOPMOST, 0, 0, 0, 0, SWP_FLAGS
End Sub
Public Sub ApiTextBoxTabset(Box As TextBox, TabStops() As Long)
    Dim result As Long
    Dim TabCnt As Long
    Dim i As Long
    ...
    For i = 0 To UBound(TabStops)
        TabStops(i) = TabStops(i) '* 12  ' convert chars to dialog units
    Next i

    ' clear existing tab stops
    result = SendMessageSetTabs(Box.hwnd, EM_SETTABSTOPS, 0, vbNullString)

    TabCnt = UBound(TabStops) + 1
    result = SendMessageSetTabs(Box.hwnd, EM_SETTABSTOPS, TabCnt, TabStops(0))

    'result = SendMessageA(Box.hwnd, EM_SETTABSTOPS, TabCnt, VarPtr(TabStops(0)))
End Sub
...
Public Function ApiDriveArray(Optional SkipDisks As String = "") As String()
    ...
    For i = 0 To UBound(Temp)
        ...
        If Len(Temp(i)) > 0 Then
            If InStr(1, SkipDisks, Left(Temp(i), 1), vbTextCompare) <= 0 Then
                ReDim Preserve drv(inx)
                drv(inx) = Temp(i)
                inx = inx + 1
            End If
        End If
...
Private Function ApiDriveIsRemovable(DriveType As PT_DRIVE_TYPE) As Boolean
    ...
    End If
End Function
Public Function ApiDriveInfo(Drive As String, Optional QueryEvenIfRemovable As Boolean = False) As PT_DRIVE_INFO
    Dim s As String
    Dim uba() As Byte
    Dim tuba() As Byte
    Dim fs As String
    ...
    di.ClusterSize = CDec(INVALID_FILE_SIZE)
    di.DriveIsCompressed = False
    di.DriveSize = CDec(INVALID_FILE_SIZE)
    di.EncryptionSupported = False
    di.Error = False
    di.ErrorDesc = ""
    di.FileSystem = ""
    di.FreeSpace = CDec(INVALID_FILE_SIZE)
    ...
    di.QuotaDisplayName = ""
    di.QuotaDomain = ""
    di.QuotaFree = CDec(INVALID_FILE_SIZE)
    di.QuotaLimit = CDec(INVALID_FILE_SIZE)
    di.QuotaSID = ""
    di.QuotaState = DISKQUOTA_STATE_UNKNOWN
    di.QuotaSupported = False
    di.QuotaUsed = CDec(INVALID_FILE_SIZE)
    di.QuotaWarning = CDec(INVALID_FILE_SIZE)
    ...
    If (Not di.Removable) Or QueryEvenIfRemovable Then
        fs = String(17, Chr(0))
    ...
    If (di.DriveType = DRIVE_REMOTE) And (Left(di.RootFolder, 2) <> "\\") Then
        s = Left(di.RootFolder, Len(di.RootFolder) - 1)
        If WNGCW_OK And (Not ApiForceAscii) Then
            ReDim uba(2048)
            Cnt = (UBound(uba) / 2) - 1
            tuba = s & vbNullChar
...
Private Function ApiShareInfo(ShareName As Variant) As PT_SHARE_INFO
    ...
    If WinVer.PlatformNT And (Not ApiForceAscii) Then
...
Public Function ApiComputerName() As String
    ...
    If GCNW_OK And (Not ApiForceAscii) Then
        ReDim uba(MAX_PATH * 2)
    ...
    ApiLastError = 0
End Function
Public Sub ApiSetShield(Ctrl As Control)
    SendMessageA Ctrl.hwnd, BCM_SETSHIELD, 0, &HFFFFFFFF
End Sub
Public Function ApiIsUserInAdminGroup() As Boolean
    ApiIsUserInAdminGroup = IsUserAnAdmin()
    Exit Function
End Function
Public Function ApiIsUserAdmin() As Boolean
    ...
    Dim Cnt As Long
    Dim lim As Long
    Dim AcctName1 As String
    Dim AcctName2 As String
    Dim DomainName As String
    Dim peUse As Long

    ApiIsUserAdmin = False

    hProcessID = GetCurrentProcess()
    If hProcessID = 0 Then Exit Function ' can't go any further :(

    result = OpenProcessToken(hProcessID, TOKEN_READ, hToken)
    If result <> 1 Then
    ...
    tilen = 0
    result = GetTokenInformation(hToken, TokenGroups, TG, tilen, buflen)
    If (result = 0) And (buflen > 0) Then
        tilen = buflen
        result = GetTokenInformation(hToken, TokenGroups, TG, tilen, buflen)
        If (result = 1) And (tilen > 0) Then
            SIA.Value(5) = SECURITY_NT_AUTHORITY
            result = AllocateAndInitializeSid(SIA, 2, SECURITY_BUILTIN_DOMAIN_RID, _
                DOMAIN_ALIAS_RID_ADMINS, 0, 0, 0, 0, 0, 0, sid)

            If result = 1 Then
                AcctName1 = Space(255)
                DomainName = Space(255)
                ' get SIDs for ADMIN groups
                result = LookupAccountSidA(vbNullString, sid, AcctName1, _
                    Len(AcctName1), DomainName, Len(DomainName), peUse)
                ...
                If result = 1 Then
                    ...
                    For Cnt = 0 To lim
                        AcctName2 = Space(255)
                        DomainName = Space(255)
                        result = LookupAccountSidA(vbNullString, _
                            TG.Groups(Cnt).sid, AcctName2, Len(AcctName2), _
                            DomainName, Len(DomainName), peUse)

                        If StrComp(AcctName1, AcctName2, vbTextCompare) = 0 Then
                            ApiIsUserAdmin = True
                            Exit For
                        End If
                    Next
    ...
    If sid <> 0 Then FreeSid sid
    If hToken <> 0 Then CloseHandle hToken
    If hProcessID <> 0 Then CloseHandle hProcessID
End Function
Public Function ApiUserName() As String
    ...
    If GUNW_OK And (Not ApiForceAscii) Then
        ReDim uba(MAX_PATH * 2)
...
Public Function ApiFolderWinSys() As String
    ...
    If GSDW_OK And (Not ApiForceAscii) Then
        ReDim uba(MAX_PATH * 2)
...
End Function
Public Function ApiFolderWin() As String
    Dim uba() As Byte
    ...
    If GSDW_OK And (Not ApiForceAscii) Then
        ReDim uba(MAX_PATH * 2)
        result = GetWindowsDirectoryW(uba(0), MAX_PATH)
        If result = 0 Then
            SetApiError
            WinDir = UNKNOWN_LIT
            Exit Function
    ...
    Else
        ...
        result = GetWindowsDirectoryA(WinDir, MAX_PATH)
        If result = 0 Then
            SetApiError
            WinDir = UNKNOWN_LIT
            Exit Function
        End If
    End If
    ApiFolderWin = ApiTextStrip(WinDir)
End Function
...
End Function
Public Function ApiTextHeightTwips(hdc As Long, Text As String) As Single
    Dim result As Long
    Dim xySize As PT_SIZE

    result = GetTextExtentPoint32A(hdc, Text, Len(Text), xySize)
    ApiTextHeightTwips = xySize.HeightPixels * Screen.TwipsPerPixelY
End Function
Public Function ApiTextWidthTwips(hdc As Long, Text As String) As Single
    Dim result As Long
    Dim xySize As PT_SIZE

    result = GetTextExtentPoint32A(hdc, Text, Len(Text), xySize)
    ApiTextWidthTwips = xySize.WidthPixels * Screen.TwipsPerPixelX
End Function
Public Function ApiTextHeightPixels(hdc As Long, Text As String) As Single
    Dim result As Long
    Dim xySize As PT_SIZE

    result = GetTextExtentPoint32A(hdc, Text, Len(Text), xySize)
    ApiTextHeightPixels = xySize.HeightPixels
End Function
Public Function ApiTextWidthPixels(hdc As Long, Text As String) As Single
    Dim result As Long
    Dim xySize As PT_SIZE

    result = GetTextExtentPoint32A(hdc, Text, Len(Text), xySize)
    ApiTextWidthPixels = xySize.WidthPixels
End Function
...
Private Function WordWrapPara(hdc As Long, MaxWidthPixels As Long, Para As String, ByRef ta As PT_TEXT_ARRAY) As PT_TEXT_ARRAY
    ...
    Else
        ta.LineTable(ta.LineCnt) = Left(Para, FitCnt)
        NextBeg = FitCnt + 1
    End If
...
End Function
Public Function ApiPrinterFit(PrnObj As Object, Width As Single, WidthUnit As ScaleModeConstants, Text As String) As String
    Dim result As Long
    Dim ubuf() As Byte
    Dim abuf As String
...
'    WidthPixels = Width / PrnObj.TwipsPerPixelX
    WidthPixels = PrnObj.ScaleX(Width, WidthUnit, vbPixels)

    If SHPCPW_OK And (Not ApiForceAscii) Then
        ubuf = Text & vbNullChar
        ReDim Preserve ubuf(MAX_PATHW + 20)
        result = PathCompactPathW(PrnObj.hdc, ubuf(0), WidthPixels)
        abuf = ubuf
        ApiPrinterFit = ApiTextStrip(abuf)
    ElseIf SHPCP_OK Then
        abuf = Text & String(MAX_PATH - Len(Text) + 2, 0)
        result = PathCompactPathA(PrnObj.hdc, abuf, WidthPixels)
        ApiPrinterFit = ApiTextStrip(abuf)
    Else
        ApiPrinterFit = Text
    End If
End Function
Public Function ApiTextPathFit(hdc As Long, WidthTwips As Long, Text As String) As String
    Dim result As Long
    Dim ubuf() As Byte
    Dim abuf As String
    Dim WidthPixels As Long
    ...
    WidthPixels = WidthTwips / Screen.TwipsPerPixelX

    If SHPCPW_OK And (Not ApiForceAscii) Then
        ubuf = Text & vbNullChar
        ReDim Preserve ubuf(MAX_PATHW + 20)
        result = PathCompactPathW(hdc, ubuf(0), WidthPixels)
        abuf = ubuf
        ApiTextPathFit = ApiTextStrip(abuf)
    ElseIf SHPCP_OK Then
        abuf = Text & String(MAX_PATH - Len(Text) + 2, 0)
        result = PathCompactPathA(hdc, abuf, WidthPixels)
        ApiTextPathFit = ApiTextStrip(abuf)
    Else
        ApiTextPathFit = Text
    End If
...
End Function
Public Function ApiConvISO8601Date(ISO As String) As Date
    Dim i As Long
    Dim ds As String
    Dim ts As String
    Dim sa() As String
    ...
    i = InStr(1, ISO, "T", vbTextCompare)
    If i > 0 Then
        ds = Left(ISO, i - 1)
        ts = Mid(ISO, i + 1)
    Else
        If StrComp(Left(ISO, 1), "T", vbTextCompare) = 0 Then
            ts = Mid(ISO, 2)
            ds = ""
        Else
            ds = ISO
    ...
    lim = UBound(sa)
    If 0 <= lim Then
        If IsNumeric(sa(0)) Then dt = DateSerial(sa(0), Month(dt), Day(dt)) + TimeSerial(Hour(dt), Minute(dt), Second(dt))
        If 1 <= lim Then
            If IsNumeric(sa(1)) Then dt = DateSerial(Year(dt), sa(1), Day(dt)) + TimeSerial(Hour(dt), Minute(dt), Second(dt))
            If 2 <= lim Then
                If IsNumeric(sa(2)) Then dt = DateSerial(Year(dt), Month(dt), sa(2)) + TimeSerial(Hour(dt), Minute(dt), Second(dt))
            End If
        End If
    End If
    ...
    lim = UBound(sa)
    If 0 <= lim Then
        If IsNumeric(sa(0)) Then dt = DateSerial(Year(dt), Month(dt), Day(dt)) + TimeSerial(sa(0), Minute(dt), Second(dt))
        If 1 <= lim Then
            If IsNumeric(sa(1)) Then dt = DateSerial(Year(dt), Month(dt), Day(dt)) + TimeSerial(Hour(dt), sa(1), Second(dt))
            If 2 <= lim Then
                If IsNumeric(sa(2)) Then dt = DateSerial(Year(dt), Month(dt), Day(dt)) + TimeSerial(Hour(dt), Minute(dt), sa(2))
            End If
        End If
    End If

    ApiConvISO8601Date = dt
End Function
Public Function ApiConvISO8601Text(ISO As String) As String
    Dim i As Long
    Dim ds As String
    Dim ts As String
    Dim sa() As String
    ...
    ApiConvISO8601Text = dt
End Function
Public Function ApiIsArray(Data As Variant) As Boolean
    ApiIsArray = False
    ...
    End If
End Function
Public Function ApiConvByteArrayUDec(ByRef ByteArray() As Byte, Optional BigEndian As Boolean = False) As Variant
    Dim Power As Long
    Dim i As Long
    Dim Dec As Variant
    Dim b() As Byte
    ...
    For i = 0 To lim
        Dec = Dec + CDec((CLng(b(i)) * (256 ^ i)))
    Next i

    ApiConvByteArrayUDec = Dec
End Function
Public Function ApiConvByteArraySDec(ByRef ByteArray() As Byte, Optional BigEndian As Boolean = False) As Variant
    Dim Power As Long
    Dim i As Long
    Dim Dec As Variant
    Dim ba() As Byte
...
'        b(lim) = b(lim) And &H7F
    ...
    If Neg Then
        Comp = CDec(0)
        For i = 0 To lim
            Comp = Comp + CDec((CLng(&HFF) * (256 ^ i)))
        Next i
        Dec = Dec - Comp - 1
'        dec = dec * (-1)
    End If

    ApiConvByteArraySDec = Dec
End Function
...
End Function
Public Function ApiDispHex(DataArray() As Byte, Optional GroupSize As Long = 0) As String
    Dim i As Long
    Dim lim As Long
    Dim s As String
    Dim c As String
...
Public Function ApiDispDHMS(ms As Variant, Optional ShortLevel As SHORT_LEVEL = SHORT_NONE) As String
    ...
    s = ""
    If (days > 0) Or (Not Shorten) Or (Len(s) > 0) Then
        If Len(s) > 0 Then s = s & ", "
        ...
        End If
    End If
    If (hrs > 0) Or (Not Shorten) Or (Len(s) > 0) Then
        If Len(s) > 0 Then s = s & ", "
        ...
        End If
    End If
    If (mins > 0) Or (Not Shorten) Or (Len(s) > 0) Then
        If Len(s) > 0 Then s = s & ", "
        ...
        End If
    End If
    If (secs > 0) Or (Not Shorten) Or (msecs > 0) Or (Len(s) > 0) Then
        If Len(s) > 0 Then s = s & ", "
